...
class 文件结构和 dex 文件结构，分析各自优劣，以及移动端的特点

class 文件：文件头、常量池、类信息、字段表、方法表、属性信息

dex 文件把多个 class 文件变成 1 个 dex 文件

class 文件 vs dex 文件

- class 文件每个类都有独立的常量池，导致代码冗余
- dex 文件常量记录在数据区，所有类都通过索引获取常量的值，相同的常量可以合并，大大降低了代码的冗余
- class 文件每个类属于一个文件
- dex 文件所有类在同一个文件中，即使是 multidex，文件总数也远小于 class 文件。降低I/O操作开销
- class 文件是基于栈的字节码
- dex 文件基于虚拟寄存器对字节码

移动端内存有限，安装包体积不宜太大，频繁I/O操作会带来卡顿
...
能写 java.lang.Object，能通过编译，但不会被加载，想要被加载就要打破双亲委派模型
...
1. 面试官既然这样问，说明肯定有例外
2. 实例不能常驻内存，如果不在堆中，就只能是栈中
3. 如果 JVM 支持把实例放在栈中，那肯定有相应好处

1. 堆和栈的区别是什么
2. 实例放在栈中的好处是什么
3. 什么情况下，实例适合放在栈中

把堆中的数据，等量平移到栈的局部变量表中的操作叫：标量替换

在栈上分配空间，存放实例叫栈上分配

好处：实例出栈即销毁，不依赖GC，没有额外开销
...
- 早期，全栈回收（没有分区和分代），没有并发（会STW），平均一次GC STW时间在100ms
- Android2.3之后，有了并行回收（CMS），有了分区回收，平均一次GC STW时间在5ms

ART GC的策略

##### 双重检测的单例，为什么还要加volatile关键字？

原子性：一个或一组操作是不可分割的原子，不能打断

- 可以通过 synchronized 和一些 Lock 来保证

可见性：一个线程修改变量后，立即能被另一个线程观测到

- volatile 关键字要求变量被修改后立即同步到主内存，每次使用也必须从主内存读取
- synchronized 保证，在加锁前清空工作内存的变量值，从主内存重新读取，在释放锁前把工作内存变量刷新回主内存
